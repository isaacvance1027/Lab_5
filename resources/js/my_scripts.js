...
function loadStatsPage(){
	...
	win_element = document.getElementById('wins');
	var wins = 0;
	var cell_1;
	var vell_2;

	for (i=2	; i < table.rows.length; i++){
		cell_1 = parseInt(table.rows[i].cells[2].innerHTML);
		cell_2 = parseInt(table.rows[i].cells[3].innerHTML);
	...
	win_element.innerHTML = wins;
}



function loadPlayersPage(){
	document.getElementById('player_selector').innerHTML = players.map((item,index) => `
			<div href="#{$item.name}" onclick="switchPlayers(${index})" class="dropdownItem">
			${item.name}
			</div>
			`
		).join(" ");
